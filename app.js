...
function cfSnapshotData() {
    return JSON.stringify({listaBancos,listaTarjetas,listaServicios,listaCorrientes,listaRubros,
        ...
        listaServiciosUSD,listaCorrientesUSD,tipoCambio,
...
// ═══════════════════════════════════════════
//  BACKUP AL SALIR (snapshot local + carpeta local)
// ═══════════════════════════════════════════
...
async function syncAlSalir() {
    ...
    const lineas = [
        ...
        (carpetaEstado === null ? '➖ Carpeta local: no vinculada (usá 📂 Carpeta para vincular una)' : (carpetaEstado ? '✅ Carpeta local: guardado' : '❌ Carpeta local: error'))
    ];
    alert(lineas.join('\n'));
    window.close();
}
...
async function cfBackupEnCarpeta(handle) {
    ...
    try {
        ...
        const data   = {listaBancos,listaTarjetas,listaServicios,listaCorrientes,listaRubros,
                        ...
                        listaPresupRubros,listaPresupRubrosUSD,
...
function abrirModalPagoTarjeta(tarjetaId) {
    ...
    if (!t) return;
    const prev = document.getElementById('cf-modal-overlay');
    if (prev) prev.remove();
    ...
    <div style="background:#1e293b;border-radius:14px;width:100%;max-width:420px;padding:20px 18px 24px;box-shadow:0 8px 40px rgba(0,0,0,0.6);color:#f1f5f9;">
        ...
        <div style="display:flex;gap:8px;">
            <button onclick="cfCerrarModal()" style="flex:1;padding:10px;border-radius:10px;font-size:12px;font-weight:600;border:1.5px solid #334155;background:#0f172a;color:#94a3b8;cursor:pointer;">✕ Cancelar</button>
        </div>
    </div>`;
...
function cfCerrarModal() {
    const ov = document.getElementById('cf-modal-overlay');
    if (ov) ov.remove();
}
function cfToast(msg, esError = false) {
    const prev = document.getElementById('cf-toast');
    if (prev) prev.remove();
    const t = document.createElement('div');
    t.id = 'cf-toast';
    t.style.cssText = 'position:fixed;bottom:20px;left:50%;transform:translateX(-50%);background:'+(esError?'#dc2626':'#1e293b')+';color:#f1f5f9;padding:10px 18px;border-radius:10px;font-size:13px;font-weight:600;box-shadow:0 4px 20px rgba(0,0,0,0.4);z-index:2147483647;max-width:90vw;text-align:center;';
    t.innerText = msg;
